...
/* NEW: fallback geocenters for city/county, same approach as other pages */
...
export default function EventCreate() {
    ...
    /* Resolve coordinates:
       1) If address is present, use Google Geocode proxy (/api/geocode)
       2) Else if city is present, use city center
       3) Else if county is present, use county center
    */
    const resolveCoordsForSave = async () => {
        ...
        // 1) address → Google Geocode proxy
        const addrStr = (address || '').trim();
        const cityStr = (city || '').trim();
        if (addrStr) {
            try {
                const body = { street: addrStr, city: cityStr, state: 'AL', country: 'US' };
                ...
                // fall through to city/county
            }
        }

        // 2) city center
        if (cityStr) {
            const obj = cityData.find((c) => c.name.toLowerCase() === cityStr.toLowerCase());
            if (obj?.coordinates?.length === 2) {
                return { lat: Number(obj.coordinates[0]), lng: Number(obj.coordinates[1]) };
            }
        }
        ...
        if (countyStr) {
            const full = countyStr.toLowerCase().endsWith(' county') ? countyStr : `${countyStr} County`;
            const obj = countyData.find((c) => c.name.toLowerCase() === full.toLowerCase());
            if (obj?.coordinates?.length === 2) {
                return { lat: Number(obj.coordinates[0]), lng: Number(obj.coordinates[1]) };
            }
        }
    ...
    const onSubmit = async () => {
        ...
        setSubmitting(true);
        try {
            // NEW: auto-resolve coordinates if missing
            const coords = await resolveCoordsForSave();
            const latOut = coords.lat != null ? Number(coords.lat) : null;
            const lngOut = coords.lng != null ? Number(coords.lng) : null;
            ...
            const payload = {
                ...
                city: city || null,
                county: county || null,
                lat: latOut,           // ← now always included when available
                lng: lngOut,           // ← now always included when available
                is_online: !!isOnline,
                is_free: !!isFree,
    ...
    return (
        <Box sx={{ p: { xs: 1.5, md: 2.5 } }}>
            <Paper variant="outlined" sx={{ p: 2 }}>
                <Typography variant="h6" fontWeight={700} sx={{ mb: 1 }}>Create a New Event</Typography>
                <Divider sx={{ mb: 2 }} />
                ...
                <Stack spacing={2}>
                    ...
                    <TextField select label="Category" value={category} onChange={(e)=>setCategory(e.target.value)} helperText="Choose the closest match">
                    ...
                    <Stack direction={{ xs:'column', sm:'row' }} spacing={2}>
                        <TextField type="datetime-local" label="Start (local) *" value={startLocal} onChange={(e)=>setStartLocal(e.target.value)} InputLabelProps={{ shrink: true }} fullWidth helperText="Local time is converted to UTC on save" />
                        <TextField type="datetime-local" label="End (local)" value={endLocal} onChange={(e)=>setEndLocal(e.target.value)} InputLabelProps={{ shrink: true }} fullWidth />
                    </Stack>
