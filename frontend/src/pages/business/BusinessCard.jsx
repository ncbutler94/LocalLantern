...
// ↓ Hero image just a bit shallower (was '16 / 9')
const HERO_ASPECT = '18 / 9';
// Slightly shorter content area (you set this previously)
const BODY_MIN_HEIGHT = 175;

const TITLE_LINES = 2;
const SNIPPET_CHARS = 70;
const SNIPPET_LINES = 2;
// Reserve a touch of room so “... more” has space inside the clamp
const SNIPPET_BACKOFF = 10;
...
export default function BusinessCard({
    ...
    // Build a snippet (slightly backed off so “… more” fits well)
    const descPlain = stripHtmlToText(biz.description || '');
    const showMore = descPlain.length > SNIPPET_CHARS;
    const snippet = showMore
        ? descPlain.slice(0, Math.max(0, SNIPPET_CHARS - SNIPPET_BACKOFF)).trim()
        : descPlain;
    ...
    return (
        <Card
            ...
            sx={{
                ...
                bgcolor: isHovered && !overAddress ? 'grey.100' : 'background.paper',
        ...
        >
            ...
            >
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mb: 0.5, minWidth: 0 }}>
                    {cover ? (
                        <Avatar src={logo || undefined} alt={biz.name} sx={{ width: 52, height: 52 }}>
                            {(biz.name || 'B').slice(0, 1)}
                        </Avatar>
                    ) : null}
                ...
                <Box
                    ...
                    sx={{ mt: 'auto', pt: 0.5, color: 'text.secondary', cursor: 'pointer' }}
                >
