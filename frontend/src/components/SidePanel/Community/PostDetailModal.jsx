...
// ============================================================================
// Threaded replies v9
// - Comments header padded away from the close X
// - True horizontal scroll (inner wrapper + non‑shrinking editors)
// - Show‑replies label never clipped; viewport padded on the right
// - Main composer spans full width
// - Single photo scaled down to a nicer max size
// - Image lightbox close button fixed (z-index + pointer events)
// - Safe effects (no async returned) to avoid destroy() errors in useEffect
// ============================================================================
...
const SINGLE_IMAGE_MAX = 720; // px — keeps single images from feeling “blown up”
        ...
        const run = async () => {
            ...
            try {
                ...
                const arr = (j.results || []).map(r => ({
                    ...
                    gif:  r.media_formats?.gif?.url     || r.media_formats?.tinygif?.url
                }));
...
/* Inline ReplyEditor (embedded here so we can tweak width rules for nested vs main) */
function InlineReplyEditor({
                               avatar, onSubmit, autoFocus=false, initialText='', placeholder='Write a reply…',
                               fullWidth=false
                           }) {
        ...
        <Box sx={{ display:'flex', gap:1, mt:1, minWidth: 0 }}>
            ...
            <Paper
                ...
                sx={{
                    p: 1,
                    // IMPORTANT:
                    // - Nested editors must not shrink (to force horizontal overflow & scrollbar)
                    // - Main composer should fill available width
            ...
            <span>
              <IconButton size="small" component="label" disabled={Boolean(gifUrl)}>
                ...
                <TextField
                    inputRef={textRef}
                    multiline minRows={2} maxRows={6} variant="standard" fullWidth
                ...
                {gifUrl && (
                    <Box sx={{ mt:1, position:'relative', width:160 }}>
                        ...
                        <IconButton size="small" onClick={removeGif}
                                    sx={{ position:'absolute', top:-8, right:-8, bgcolor:'#fff' }}>
                            <CloseIcon fontSize="small"/>
                        </IconButton>
                    </Box>
...
export default function PostDetailModal({ open, post, onClose, user, currentUser }) {
    ...
    const viewerProp = user || currentUser || null;
    const [viewer, setViewer] = useState(viewerProp);
    useEffect(()=>{ setViewer(viewerProp); }, [viewerProp]);
    ...
    const [validPhotos, setValid] = useState([]);
    const [imgReady, setImgReady] = useState(false);
    const [idx, setIdx]     = useState(0);
    const [lightbox, setLightbox] = useState(false);

    // remember which threads are expanded so they stay open after updates
    ...
    // validate post photos
    useEffect(() => {
        let cancel = false;
        if (!photos?.length) { setValid([]); setImgReady(true); return; }
        setImgReady(false);
        let ok=[],done=0;
        photos.forEach(src => {
            const img = new Image();
            const finish = () => { if(++done===photos.length && !cancel){ setValid(ok); setImgReady(true); } };
            img.onload  = () => { ok.push(src); finish(); };
            img.onerror = finish;
            img.src = src;
        });
        return () => { cancel = true; };
    ...
    useEffect(() => {
        ...
        const run = async () => {
            setBusyC(true);
            try {
                const r = await fetch(`/api/posts/${postId}/comments?sort=${sort}`, { credentials: 'include' });
                const j = await r.json();
    ...
    const postComment = async (fd, done) => {
        ...
        try {
            ...
            const j = await r.json();
            if (j && j.id) setComments(c => [j, ...c]); // add without refetching
            done();
        } catch (e) { console.error(e); }
    };
    ...
    // image collage (with nicer single-photo sizing)
        ...
        return (
            <Box sx={{ ...wrapperSx, display:'grid', gap:0.5, gridTemplateColumns:tpl.c, gridTemplateRows:tpl.r }}>
                {pics.slice(0,4).map((src,i)=>(
                    <Box key={src} onClick={()=>onClick(i)} sx={{ position:'relative', pb:n===1?'66.66%':'100%', overflow:'hidden', cursor:'pointer', borderRadius:2, ...pos(i) }}>
    ...
    const ensureIntoView = (el) => {
        ...
        const dx = r.right > v.right ? (r.right - v.right + 16)
            : r.left  < v.left  ? -(v.left - r.left  + 16) : 0;
        const dy = r.bottom > v.bottom ? (r.bottom - v.bottom + 16)
            : r.top    < v.top    ? -(v.top    - r.top    + 16) : 0;
                    ...
                    <Box sx={{ flex:1, minWidth: 0 }}>
                        ...
                        <Box sx={{ display:'flex', alignItems:'center', gap:2, mt:0.5, flexWrap:'wrap' }}>
                            ...
                            <Button size="small" sx={{ textTransform:'none', p:0 }}
                                    onClick={()=> viewer ? setReplying(true) : openAuth()}>
                                Reply
                            </Button>
                            ...
                            {kids.length>0 && (
                                <Button
                                    ...
                                    sx={{
                                        textTransform:'none', p:0, display:'inline-flex', alignItems:'center', gap:0.25,
                                        whiteSpace:'nowrap' // keep "(n)" visible; no wrapping/clipping
                                    }}
                                >
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    {/* header — extra right padding so "Most Liked" doesn’t sit under the X */}
                    <Box
                        sx={{
                            px:2, pt:2, pb:1,
                            pr:12, // <-- keep clear of the close button
                    ...
                    <Box
                        ...
                        sx={{
                            overflowY:'auto',
                            overflowX:'auto',                 // horizontal scroll for deep nesting
                            px:2, py:1, pr:3,                 // pr keeps right-edge content (e.g., "(3)") visible
                            scrollbarGutter: 'stable both-edges'
                        }}
                    >
                        {/* force horizontal overflow when thread is wider than viewport */}
                    ...
                    <Box sx={{ borderTop:1, borderColor:'divider', px:2, py:1, display:'flex', gap:2, alignItems:'center', bgcolor:'background.paper' }}>
                        ...
                        <IconButton onClick={()=>navigator.share?.({title, text:(description||'').slice(0,140), url: window.location.href})}><ShareIcon/></IconButton>
                    </Box>

                    {/* composer (fixed, full width) */}
                    <Box id="post-reply-anchor" sx={{ borderTop:1, borderColor:'divider', p:1, bgcolor:'background.paper' }}>
                        {viewer ? (
                            <InlineReplyEditor
                                ...
                                fullWidth={false}   // <-- fills the entire footer width
                            />
                        ) : (
                    ...
                    <DialogContent sx={{ p:0, bgcolor:'grey.900', position:'relative' }}>
                        <IconButton
                            ...
                            sx={{
                                ...
                                zIndex:3,               // ensure click target wins
                                pointerEvents:'auto'
                            }}
                        >
